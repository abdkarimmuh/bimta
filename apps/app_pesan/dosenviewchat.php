...
<?php 
	$id = $_GET['id'];
	$edit = $db_con->query("SELECT chat_mhs.*, subject.subject, mahasiswa.nm_mhs, prodi.nama_prodi FROM chat_mhs, subject, mahasiswa, prodi WHERE chat_mhs.id_subject=subject.id_subject && id_chatmhs='$id' && chat_mhs.mhs_pengirim=mahasiswa.nim && mahasiswa.id_prodi=prodi.id_prodi");
...
<p>From : <?php echo $row['nm_mhs']; ?> | <?php echo $row['nama_prodi']; ?></p>
<p class="color_text"><i class="fa fa-calendar-o"></i> <?php $tgl=date_create($row['tgl']); echo date_format($tgl, 'd F Y, H:i'); ?></p>
</div>

<?php							
	$stmt = $db_con->prepare("SELECT comment_bimbingan.*, chat_mhs.id_chatmhs, dosen.nm_dosen, mahasiswa.nm_mhs FROM comment_bimbingan, chat_mhs, dosen, mahasiswa where  comment_bimbingan.id_chatmhs=chat_mhs.id_chatmhs && comment_bimbingan.id_chatmhs='$id' && chat_mhs.id_chatmhs='$id' && dosen.id_dosen='$_SESSION[user_session]' && mahasiswa.id_dosen=dosen.id_dosen && mahasiswa.nim=chat_mhs.mhs_pengirim ORDER BY id_comment ASC");
	$stmt->execute();
	$no=1;
