<div class="panel panel-default">
	<div class="panel-heading">
		<span>
			<h3>Komentari Pesan Tambahan</h3>
		</span>
...
<?php 
	$id = $_GET['id'];
	$edit = $db_con->query("SELECT pesan_tambahan.*, dosen.nm_dosen FROM pesan_tambahan, dosen WHERE id_pesantambahan='$id' && pesan_tambahan.dosen_pengirim=dosen.id_dosen");
...
<p><?php $tgl=date_create($row['tgl']); echo date_format($tgl, 'd F Y, H:i'); ?> | From : <?php echo $row['nm_dosen']; ?></p>
</div>

<?php							
	$stmt = $db_con->prepare("SELECT comment_tambahan.*, pesan_tambahan.id_pesantambahan, dosen.nm_dosen, mahasiswa.nm_mhs FROM comment_tambahan, pesan_tambahan, dosen, mahasiswa where comment_tambahan.id_pesantambahan = pesan_tambahan.id_pesantambahan && comment_tambahan.id_pesantambahan='$id' && mahasiswa.nim='$_SESSION[user_session]' && mahasiswa.id_dosen=dosen.id_dosen ORDER BY id_comment ASC");
	$stmt->execute();
	$no=1;
